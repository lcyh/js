...
function handleGetData() {
  ...
  console.log(
    "currentSelectedFirstMenu",
    JSON.stringify(currentSelectedFirstMenu, null, 2)
  );
}
// handleGetData();

function matchPath(list, currentPath = "/demo/detail") {
  let result = [];
  for (let item of list) {
    //关键代码startsWith/includes
    if (currentPath.includes(item.path)) {
      item.children = matchPath(item.children, currentPath);
      result.push(item);
    }
  }
  return result;
...
console.log("matchPath", JSON.stringify(matchPath(menuData), null, 2));
